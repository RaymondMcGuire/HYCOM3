          ...
          <el-col :span="10">
            ...
            <el-form-item>
              <el-input
                v-model="input.bi"
                type="text"
                :placeholder="`${explainText}${index+1}:明流隧洞底宽bi`"
              />
...
// 王俊勇方法计算隧洞掺气水深
@Component({
  name: 'DynamicWjyParams',
  components: {

  }
})

export default class DynamicWJYParams extends Vue {
 ...
 public addField() {
   this.params.push({ hi: '', bi: '', vi: '' })
 }

 public addFieldWithData(h:number, b:number, v:number) {
   this.params.push({ hi: h, bi: b, vi: v })
 }
