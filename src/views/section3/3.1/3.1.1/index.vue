...
// 局部
import DynamicRangeParams from './components/DynamicRangeParams.vue'
import DynamicNumberParams from './components/DynamicNumberParams.vue'

import DynamicJskSelectParams from './components/DynamicJSKSelectParams.vue'
import DynamicYxhwgParams from './components/DynamicYXHWGParams.vue'
import DynamicYxjwgParams from './components/DynamicYXJWGParams.vue'

import DynamicJkmcRangeParams from './components/DynamicJKMCRangeParams.vue'
import DynamicCkmcRangeParams from './components/DynamicCKMCRangeParams.vue'

import DynamicJxbyxjsgNumberParams from './components/DynamicJXBYXJSGNumberParams.vue'
import DynamicYxbjxjsgNumberParams from './components/DynamicYXBJXJSGNumberParams.vue'
import DynamicHxgzmParams from './components/DynamicHXGZMParams.vue'
import DynamicJklwsSelectParams from './components/DynamicJKLWSSelectParams.vue'
import DynamicYxjkdParams from './components/DynamicYXJKDParams.vue'
import DynamicYxjsdParams from './components/DynamicYXJSDParams.vue'
import DynamicDmtkParams from './components/DynamicDMTKParams.vue'
import DynamicDmtsParams from './components/DynamicDMTSParams.vue'

import DynamicPbmParams from './components/DynamicPBMParams.vue'

import DynamicCsklrskNumberParams from './components/DynamicCSKLRSKNumberParams.vue'
import DynamicCsklrmqParams from './components/DynamicCSKLRMQParams.vue'

// 沿程
import DynamicJkdParams from './components/DynamicJKDParams.vue'
import DynamicJkjbdParams from './components/DynamicJKJBDParams.vue'
import DynamicCkjbdParams from './components/DynamicCKJBDParams.vue'
import DynamicDsdParams from './components/DynamicDSDParams.vue'

import { crossSectionShapeType, Table2DLerp, TABLE413_DATA, TABLE413_HORIZON, TABLE413_VERTICAL, TABLE414_DATA, TABLE414_HORIZON, TABLE414_VERTICAL, TABLE415_DATA, TABLE415_HORIZON, TABLE415_VERTICAL, TABLE416_DATA, TABLE416_HORIZON, TABLE416_VERTICAL } from '@/hycom_lib/common'
...
export default class Chapter4Section2131 extends Vue {
  public title = '3.1.1 有压管道流量系数计算';
  ...
  public beforeOnDemo() {
    ...
    let dsd = this.$refs.dsd as any
    dsd.addFieldWithData(331.2, 0, 0, 9.0, crossSectionShapeType.CIRCLE)
    dsd.onParamsDataChange()
  }

  public beforeOnCalculate() {
    let ak = +this.data.A_k
    ...
    this.paramJSKData.forEach(function(elem:any) {
      ...
      // console.log("进水口",prop, elem.value)
    })
    ...
    this.paramJKMCData.forEach(function(elem:any) {
      ...
      let prop = ak / area
      localEnergy += prop * prop * elem.value
      // console.log("门槽", prop, elem.value)
    })
    ...
    // console.log('局部', localEnergy)

    // 沿程能量损失
    // 1：进水口段
    this.paramJSKDData.forEach(function(elem:any) {
      let area = 0
      let perimeter = 0
      if (elem.shape === 0) {
        area = elem.H * elem.B
      ...
      let akDivAi = ak / area
      regionEnergy += elem.l * akDivAi * akDivAi / (Math.pow(r, 4 / 3))
      // console.log("进水口段",r, akDivAi)
    })

    // 2：进口渐变段
    this.paramJKJBDData.forEach(function(elem:any) {
      let area1 = 0
      let perimeter1 = 0
      let area2 = 0
      let perimeter2 = 0
      ...
      let r = (r1 + r2) / 2
      let akDivAi = 2 * ak / (area1 + area2)
      regionEnergy += elem.l * akDivAi * akDivAi / (Math.pow(r, 4 / 3))
      // console.log("进口渐变段",r, akDivAi)
    })

    // 3：出口渐变段
    this.paramCKJBDData.forEach(function(elem:any) {
      let area1 = 0
      let perimeter1 = 0
      let area2 = 0
      let perimeter2 = 0
      ...
      let akDivAi = 2 * ak / (area1 + area2)
      regionEnergy += elem.l * akDivAi * akDivAi / (Math.pow(r, 4 / 3))
      // console.log("出口渐变段",r, akDivAi)
    })

    // 4：洞身段
    this.paramDSDData.forEach(function(elem:any) {
      let area = 0
      let perimeter = 0
      if (elem.shape === 0) {
        area = elem.H * elem.B
      ...
      let akDivAi = ak / area
      regionEnergy += elem.l * akDivAi * akDivAi / (Math.pow(r, 4 / 3))
      // console.log("洞身段",r, akDivAi)
    })

    regionEnergy = regionEnergy * 2 * 9.8 * +this.data.n_z * +this.data.n_z
    // console.log('沿程', regionEnergy)

    let mu = 1 / Math.sqrt(1 + regionEnergy + localEnergy)
    let outStr = '流量系数=' + mu.toFixed(2).toString()
